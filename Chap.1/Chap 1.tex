...
J. Schwinger发展了一套基于选择性测量的量子力学形式。他引入测量算符$M(a')$作为一个记号，也就是$\Lambda_{a'}$或者说$|a'\rangle\langle a'|$，并通过研究不同的SG型实验得到了一系列关于$M(a')$（或者更一般的$M(b',a')$表示$|b'\rangle\langle a'|$）的性质。他以此为源建立了关于右矢，左矢的一系列数学。本书不会遵循Schwinger的做法，感兴趣的读者可以参考Gottfried(1966)\footnote{...}。%left unfinished
